import { getLaneDifference } from '$lib/utils/lane';
import {
	holdItem,
	restItem,
	slideItem,
...
const holdExp = /^h(?<duration>\[(?:(?<bpm>\d+)#)?(?<division>\d+):(?<count>\d+)\])?$/;
const slideExp =
...
function parseSimaiNote(str: string): Chart['items'] {
	...
	for (const part of parts) {
		...
		if (noteMatch?.groups) {
			...
			if (hold) {
				const { duration, bpm, division, count } = hold.match(holdExp)?.groups ?? {};
				if (!duration) {
