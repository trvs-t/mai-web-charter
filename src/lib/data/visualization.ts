import type { Chart, SlideType } from './chart';
...
export type NoteVisualizationData = TapVisualizeData | HoldVisualizeData | SlideVisualizationData;
		...
		if (!Array.isArray(item)) {
			...
			switch (type) {
				case 'timeSignature':
					...
					continue;
				case 'rest':
					time += data.divisionCount * ((60000 / currentBpm / currentDivision) * 4);
					continue;
			}
		}
		// handle notes only
					...
					notes.push({
						...
						data: {
							...
							duration:
								...
								((60000 / (data.duration.bpm ?? currentBpm) / data.duration.division) * 4),
							isEach
						}
					});
					break;
				case 'slide':
					notes.push({
						...
						data: {
							lane: data.lane,
							hitTime: time,
							startTime: time + (60000 / (data.duration.bpm ?? currentBpm) / 4) * 4,
							duration:
								data.duration.divisionCount *
								((60000 / (data.duration.bpm ?? currentBpm) / data.duration.division) * 4),
